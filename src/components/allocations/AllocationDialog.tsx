...
import { useCreateAllocation, HailingServiceType } from '@/hooks/useAllocations';
...
type AllocationMode = 'fleet' | 'hailing';

export function AllocationDialog({ open, onOpenChange, request }: AllocationDialogProps) {
  const [mode, setMode] = useState<AllocationMode>('fleet');
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <ToggleGroup
              type="single"
              value={mode}
              onValueChange={(val) => { if (val) setMode(val as AllocationMode); }}
              className="w-full justify-start"
            >
              <ToggleGroupItem value="fleet" className="flex-1 gap-2">
                <Car className="h-4 w-4" />
                Fleet Vehicle
              </ToggleGroupItem>
              <ToggleGroupItem value="hailing" className="flex-1 gap-2">
          ...
          {mode === 'fleet' ? (
            ...
            /* Hailing Service Selection */
            <div className="space-y-2">
              <Label htmlFor="hailing-service">Service Provider *</Label>
              <Select value={hailingService} onValueChange={setHailingService}>
                <SelectTrigger id="hailing-service">
                  <SelectValue placeholder="Select a service" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="pickme">
                    <div className="flex items-center gap-2">
                      <span>🚕</span>
                      <span>PickMe</span>
                    </div>
          ...
          <div className="space-y-2">
            <Label htmlFor="notes">
              Notes {mode === 'hailing' ? '(booking ref, cost, etc.)' : '(optional)'}
            </Label>
            <Textarea
              id="notes"
              placeholder={mode === 'hailing' 
                ? "Booking reference, fare amount, driver contact..." 
        ...
        <DialogFooter>
          ...
          <Button 
            ...
            disabled={!canSubmit || createAllocation.isPending || isOverdue}
          >
