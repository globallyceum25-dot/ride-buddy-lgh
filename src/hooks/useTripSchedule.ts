...
export function useMonthSchedule(monthDate: Date) {
  const { user } = useAuth();
  
  return useQuery({
    queryKey: ['month-schedule', format(monthDate, 'yyyy-MM'), user?.id],
    queryFn: async () => {
      const monthStart = startOfMonth(monthDate);
      const monthEnd = endOfMonth(monthDate);
      
      // Get a bit extra for the calendar display (show full weeks)
      ...
      allocations?.forEach(allocation => {
        const dateStr = format(new Date(allocation.scheduled_pickup), 'yyyy-MM-dd');
        if (!tripData[dateStr]) {
          tripData[dateStr] = {
            total: 0,
            scheduled: 0,
        ...
        tripData[dateStr].total++;
        
        switch (allocation.status) {
          case 'scheduled':
            tripData[dateStr].scheduled++;
            break;
          case 'dispatched':
            tripData[dateStr].dispatched++;
            break;
          case 'in_progress':
            tripData[dateStr].inProgress++;
            break;
          case 'completed':
...
export function useWeekTrips(weekStart: Date, filters: TripScheduleFilters = {}) {
  const { user, hasRole } = useAuth();
  
  return useQuery({
    queryKey: ['week-trips', format(weekStart, 'yyyy-MM-dd'), filters, user?.id],
    queryFn: async () => {
